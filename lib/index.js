...
module.exports = function(app, mockUrl = '/nock') {
  const splitServices = {};
  const appConfig = {};
   ...
   */
  const getApp = (pathname) => new promisify(app, { get: function(target, name){
    if (typeof target[name] === 'function' && typeof name === 'string') {
      ...
      return function(url, callback, ...rest){
        const confg = {method: name.toUpperCase(), url: url, __mockTimeout: callback.mockTimeout, config: callback.toString(), parmars: callback.parmars, func: parmars.func};
  ...
  const getFileManage = (pathname) => (defaultMock, mocks = [], timeout = 0) => {
    const getParmars = strFn => _.uniq(strFn.math(/(?<![a-z0-1_])req\.(params|query|headers|body)\.[a-z0-1_.]+/ig) || []).filter(i => i!== 'req.headers.mockStatus').map(i => i.replace('req', ''));
  ...
  app.get(mockUrl = (req, res) => {
    const html = fs.readFileSync(path.join(__dirname, 'idex.html').toString());
